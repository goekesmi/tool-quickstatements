...
require_once ( __DIR__ . '/../lib/wikidata.php' ) ;
...
*/	
	...
	public function getOA() {
		if ( !isset($this->oa) ) {
			$this->oa = new MW_OAuth ( 'quickstatements' , '' , '', $this->sites->{$this->site} ) ;
		}
		return $this->oa ;
	}
	...
	public function isUserBlocked ( $username ) {
		...
		$url = $this->getSiteBase() . "api.php?action=query&list=blocks&format=json&bkusers=" . urlencode ( $username ) ;
...
//			if ( !isset($ret->$batch_id->commands) ) $ret->$batch_id->commands = (object) array() ;
	...
	protected function getBotAPI ( $force_login = false ) {
		...
		$api_url = $this.getSiteBase() .'api.php' ;
